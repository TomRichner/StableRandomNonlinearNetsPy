function SRNN_tseries_figure_IED_adapt(t, u_ex, r_ts, a_E_ts, a_I_ts, b_E_ts, b_I_ts, u_d_ts, params, T_plot_limits, Lya_method, sr_or_poisson, include_sum_E_I_SR, show_mean_adapt, varargin)
% SRNN_tseries_plot - Plot time series results from SRNN simulation (with adaptation plots)
%
% Inputs:
%   t        - Time vector
...
%   show_mean_adapt - boolean to control mean vs individual SFA/STD traces
...
% Set default for show_mean_adapt if not provided
if nargin < 14 || isempty(show_mean_adapt)
    show_mean_adapt = false;
end
...
hold on;
if show_mean_adapt
    % Plot mean SFA for excitatory neurons only
    if ~isempty(E_indices) && params.n_a_E > 0
        active_E_sfa = params.c_SFA(E_indices) ~= 0;
        if any(active_E_sfa)
            mean_sfa_E = mean(a_sum_plot(E_indices(active_E_sfa), plot_indices), 1);
            plot(t_display, mean_sfa_E, 'b', 'LineWidth', 2, 'DisplayName', 'Mean E SFA');
        end
    end
    % Plot mean SFA for inhibitory neurons if they have SFA states
    if ~isempty(I_indices) && params.n_a_I > 0
        active_I_sfa = params.c_SFA(I_indices) ~= 0;
        if any(active_I_sfa)
            mean_sfa_I = mean(a_sum_plot(I_indices(active_I_sfa), plot_indices), 1);
            plot(t_display, mean_sfa_I, 'r', 'LineWidth', 2, 'DisplayName', 'Mean I SFA');
        end
    end
else
...
hold on;
if show_mean_adapt
    % Plot mean STD for excitatory neurons only
    if ~isempty(E_indices) && params.n_b_E > 0
        active_E_std = params.F_STD(E_indices) ~= 0;
        if any(active_E_std)
            mean_std_E = mean(b_prod_plot(E_indices(active_E_std), plot_indices), 1);
            plot(t_display, mean_std_E, 'b', 'LineWidth', 2, 'DisplayName', 'Mean E STD');
        end
    end
    % Plot mean STD for inhibitory neurons if they have STD states
    if ~isempty(I_indices) && params.n_b_I > 0
        active_I_std = params.F_STD(I_indices) ~= 0;
        if any(active_I_std)
            mean_std_I = mean(b_prod_plot(I_indices(active_I_std), plot_indices), 1);
            plot(t_display, mean_std_I, 'r', 'LineWidth', 2, 'DisplayName', 'Mean I STD');
        end
    end
else
...
if n <= 50
    ...
    [~, ~] = plot_network_graph_widthRange_color_R(M,1,params.EI_vec);
    box off
    axis equal
    axis off
end
